...
export const MapContainer: React.FC<MapContainerProps> = ({
  ...
  useEffect(() => {
    ...
    filteredLocations.forEach((loc) => {
      ...
      const isMostConfirmed = loc.id === mostConfirmedId && (loc.confirmations_count || 0) > 0;
      const hasConfirmations = (loc.confirmations_count || 0) > 0 || loc.user_confirmed;
      const isUnconfirmedPossivel = isAndarilho && !hasConfirmations;
      const isConfirmedAndarilho = isAndarilho && hasConfirmations;

      const color = loc.color || CATEGORY_COLORS[loc.category] || '#FA7608';
      const iconSymbol = CATEGORY_ICONS[loc.category] || '📍';
    ...
    <div className="relative w-full h-full bg-slate-950 overflow-hidden select-none">
      ...
      <div className="absolute bottom-6 right-4 z-20 flex flex-col gap-2">
        <button
          ...
          className="w-10 h-10 rounded-xl bg-[#0D0D0D] backdrop-blur-md border border-[#FA7608]/25 text-slate-200 hover:text-white hover:bg-[#FA7608]/20 hover:border-[#FA7608]/60 flex items-center justify-center shadow-xl transition-all"
          title="Zoom In"
        >
        ...
        >
          <Compass className="w-5 h-5 text-[#FA7608]" />
        </button>
        <button
